...
Error BoCA::AS::DecoderComponentExternalStdIO::GetStreamInfo(const String &streamURI, Track &format)
{
	...
	do
	{
		...
		else if (chunk == "data")
		{
			...
			if (format.length == 0x3fffffff ||
			    format.length == 0x7fffffff) format.length = -1;
		}
		else
		{
			// Skip chunk
...
Bool BoCA::AS::DecoderComponentExternalStdIO::Activate()
{
	 ...
	 * if the file name contains Unicode characters.
	 */
	encFileName = format.origFilename;

	if (String::IsUnicode(format.origFilename))
	{
		encFileName = Utilities::GetNonUnicodeTempFileName(format.origFilename).Append(".").Append(specs->formats.GetFirst()->GetExtensions().GetFirst());

		File(format.origFilename).Copy(encFileName);
	}
