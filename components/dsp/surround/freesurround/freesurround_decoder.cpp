...
public:
	...
	decoder_impl(channel_setup setup, unsigned N): N(N), C((unsigned)chn_alloc[setup].size()),
		setup(setup), lt(N), rt(N), dst(N), lf(N/2+1), rf(N/2+1), forward(kiss_fftr_alloc(N,0,0,0)),
		inverse(kiss_fftr_alloc(N,1,0,0)), buffer_empty(true), inbuf(3*N), wnd(N)
	{
	...
	~decoder_impl() { kiss_fftr_free(forward); kiss_fftr_free(inverse); }
