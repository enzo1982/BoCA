...
BoCA::ConfigureCoreAudio::ConfigureCoreAudio(const CoreAudioCommCodecs &iFormats)
{
	...
	{
		if	(formats.codecs[i] == CA::kAudioFormatMPEG4AAC)		combo_codec->AddEntry("MPEG4 AAC Low Complexity");
		else if	(formats.codecs[i] == CA::kAudioFormatMPEG4AAC_HE)	combo_codec->AddEntry("MPEG4 AAC High Efficiency");
		else if	(formats.codecs[i] == CA::kAudioFormatMPEG4AAC_HE_V2)	combo_codec->AddEntry("MPEG4 AAC High Efficiency v2");
		else if	(formats.codecs[i] == CA::kAudioFormatMPEG4AAC_LD)	combo_codec->AddEntry("MPEG4 AAC Low Delay");
		else if	(formats.codecs[i] == CA::kAudioFormatMPEG4AAC_ELD)	combo_codec->AddEntry("MPEG4 AAC Enhanced Low Delay");
		else if	(formats.codecs[i] == CA::kAudioFormatMPEG4AAC_ELD_SBR)	combo_codec->AddEntry("MPEG4 AAC Enhanced Low Delay SBR");
		else if	(formats.codecs[i] == CA::kAudioFormatMPEG4AAC_ELD_V2)	combo_codec->AddEntry("MPEG4 AAC Enhanced Low Delay v2");
		else if	(formats.codecs[i] == CA::kAudioFormatMPEG4AAC_Spatial)	combo_codec->AddEntry("MPEG4 AAC Spatial");
		else if (formats.codecs[i] == CA::kAudioFormatAppleLossless)	combo_codec->AddEntry("Apple Lossless Audio Codec");
		else								continue;

		codecs.Add(formats.codecs[i]);

		if ((UnsignedInt) config->GetIntValue(ConfigID, "Codec", CA::kAudioFormatMPEG4AAC) == formats.codecs[i]) combo_codec->SelectNthEntry(combo_codec->Length() - 1);
	}
