...
class ID3_FieldImpl : public ID3_Field
{
	...
	public:
		...
		size_t			 Set(const char *buf, size_t size)	{ return this->Set(reinterpret_cast<const uchar *>(buf), size); }
		...
		bool			 InScope(ID3_V2Spec spec) const		{ return _spec_begin <= spec && spec <= _spec_end; }
		ID3_V2Spec		 MinSpec() const			{ return _spec_begin; }
		ID3_V2Spec		 MaxSpec() const			{ return _spec_end; }
