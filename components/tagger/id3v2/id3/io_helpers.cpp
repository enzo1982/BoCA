...
namespace
{
	...
	int isBOM(unsigned char ch1, unsigned char ch2)
	{
		...
		if	(ch1 == 0xFE && ch2 == 0xFF) return  1;
		else if (ch1 == 0xFF && ch2 == 0xFE) return -1;
...
String io::readUnicodeString(ID3_Reader &reader, ID3_TextEnc enc)
{ //keeps reading until null char or at end of reader end
	String			 unicode;
	ID3_Reader::char_type	 ch1, ch2;

	if (!readTwoChars(reader, ch1, ch2) || isNull(ch1, ch2)) return unicode;

	int	 bom = isBOM(ch1, ch2);

	if (!bom) reader.setCur(reader.getCur() - 2);

	while (!reader.atEnd())
	{
		if (!readTwoChars(reader, ch1, ch2) || isNull(ch1, ch2)) break;

		if (bom == 1 || (!bom && enc == ID3TE_UTF16BE))
		{
...
String io::readUnicodeText(ID3_Reader &reader, size_t len, ID3_TextEnc enc)
{
	String			 unicode;
	ID3_Reader::char_type	 ch1, ch2;

	if (!readTwoChars(reader, ch1, ch2)) return unicode;
	...
	{
		if (!readTwoChars(reader, ch1, ch2)) break;
...
size_t io::writeTrailingSpaces(ID3_Writer &writer, String buf, size_t len)
{
	...
	for (; size < len; ++size) writer.writeChar('\0');
