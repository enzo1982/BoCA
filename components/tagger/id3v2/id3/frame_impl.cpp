...
ID3_FrameImpl::ID3_FrameImpl(ID3_FrameID id) : _changed(false),
					       _bitset(),
					       _fields(),
					       _encryption_id('\0'),
					       _grouping_id('\0')
{
	this->SetSpec(ID3V2_DEFAULT);
	this->SetID(id);
}

ID3_FrameImpl::ID3_FrameImpl(const ID3_FrameHeader &hdr) : _changed(false),
							   _bitset(),
							   _fields(),
							   _hdr(hdr),
							   _encryption_id('\0'),
...
ID3_FrameImpl::ID3_FrameImpl(const ID3_Frame& frame) : _changed(false),
						       _bitset(),
						       _fields(),
						       _encryption_id('\0'),
						       _grouping_id('\0')
...
{
	for (iterator fi = _fields.begin(); fi != _fields.end(); ++fi) delete (ID3_FieldImpl*) *fi;
...
size_t ID3_FrameImpl::Size()
{
	...
	if (this->GetEncryptionID()) bytesUsed++;
	if (this->GetGroupingID()) bytesUsed++;
	...
	for (iterator fi = _fields.begin(); fi != _fields.end(); ++fi)
	{
		...
		{
			if ((*fi)->GetID() == ID3FN_TEXTENC) enc = (ID3_TextEnc) (*fi)->Get();
			else				     (*fi)->SetEncoding(enc);
...
ID3_FrameImpl &ID3_FrameImpl::operator=(const ID3_Frame &rFrame)
{
	...
	while (li != this->end())
	{
		...
		if (thisFld != NULL && thatFld != NULL) *thisFld = *thatFld;
	}
...
const char* ID3_FrameImpl::GetDescription(ID3_FrameID id)
{
	...
	if (myFrameDef != NULL) return myFrameDef->sDescription;
...
const char* ID3_FrameImpl::GetDescription() const
{
	...
	if (def) return def->sDescription;
