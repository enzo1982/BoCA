...
size_t RenderV1ToFile(ID3_TagImpl &tag, fstream &file)
{
	...
	else
	{
		...
		// the new tag.
		if (memcmp(sID, "TAG", ID3_V1_LEN_ID) == 0) file.seekp(0 - ID3_V1_LEN, ios::end);
		else					    file.seekp(0, ios::end);
	}
...
size_t RenderV2ToFile(const ID3_TagImpl &tag, fstream &file)
{
	...
	if (err != ID3E_NoError) return (size_t) err; // impossible size, will make caller be able to set _last_error
	...
	if ((!tag.GetPrependedBytes() && !ID3_GetDataSize(tag)) || (tagSize == tag.GetPrependedBytes()))
	{
	...
	else
	{
		...
		char	 sTempFile[ID3_PATH_LENGTH] = { 0 };

		strncpy(sTempFile, filename.c_str(), ID3_PATH_LENGTH - 1);
		strncat(sTempFile, sTmpSuffix.c_str(), ID3_PATH_LENGTH - strlen(sTempFile) - 1);
...
flags_t ID3_TagImpl::Update(flags_t ulTagFlag)
{
	...
	if (_last_error == ID3E_NoFile)	  _last_error = createFile(filename, file);
	if (_last_error == ID3E_ReadOnly) return tags;

	if ((ulTagFlag & ID3TT_ID3V2) && this->HasChanged())
	{
		ID3_V2Spec	 spec2use;

		if (this->UserUpdatedSpec) spec2use = this->GetSpec() < ID3V2_3_0 ? ID3V2_LATEST : this->GetSpec(); // if the spec is too old, upgrade anyway. And never use experimental ones
		else			   spec2use = ID3V2_LATEST;						    // write ID3V2_LATEST as default
		...
		if (_prepended_bytes) tags |= ID3TT_ID3V2;
	}

	if ((ulTagFlag & ID3TT_ID3V1) && (!this->HasTagType(ID3TT_ID3V1) || this->HasChanged()))
	{
		...
		if (tag_bytes)
		{
			...
			if (! _file_tags.test(ID3TT_ID3V1)) _appended_bytes += tag_bytes;
...
flags_t ID3_TagImpl::Strip(flags_t ulTagFlag)
{
	...
	if (ulTagFlag & ID3TT_PREPENDED & _file_tags.get())
	{
		...
		if (ID3E_NoError != _last_error) return ulTags;
		...
		if (!(ulTagFlag & ID3TT_APPENDED)) nBytesToCopy += this->GetAppendedBytes();
		...
		while (!file.eof())
		{
			...
			if (nBytesCopied == nBytesToCopy || nBytesToRead < BUFSIZ) break;
		}
