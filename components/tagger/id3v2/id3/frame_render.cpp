...
ID3_V2Spec ID3_FrameImpl::MinSpec() const
{
	...
	for (const_iterator cur = _fields.begin(); cur != _fields.end(); ++cur)
	{
		ID3_Field	*field = *cur;

		if (!field) continue;

		ID3_V2Spec	 fieldSpec = field->MinSpec();

		if (minSpec < fieldSpec) minSpec = fieldSpec;
	}
...
ID3_Err ID3_FrameImpl::Render(ID3_Writer& writer) const
{
	 ...
	 */
	if (!this->NumFields()) return ID3E_NoError;
	...
	else
	{
		...
		ID3D_NOTICE("ID3_FrameImpl::Render(): compressed fields, orig size = " << origSize);
	}
	...
	/* No need to not write empty frames, why would we not?
	 * They can be used to fill up padding space which is
	 * even recommended in the id3 spec.
	 */
	// if (fldSize == 0)
	// {
